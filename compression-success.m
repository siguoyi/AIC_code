...
Mt=[16,32,64,128,256,512];
successrate1=zeros(length(Mt),1);successrate2=successrate1;successrate3=successrate1;
for i=1:length(Mt)
    M=Mt(i);
...
y3=downsample(y2(N/M:N),N/M); % sample
 ...
 meanoutputSNR=zeros(1,3);   success=zeros(1,3);
for cnt = 1:length(meanoutputSNR)
       % SNR = SNRmtx;
        
        
         [x_hat1,X_hat1,freq_hat1]=rd_recovery1(y3,t,S,N,M,Tx,K); % recover input signal rd_recovery4(y,t,S,N,M,Tx)
          
         meanoutputSNR(cnt,1) =20*log10(norm(x,2)/norm((x-x_hat1),2));
          
          if  meanoutputSNR(cnt,1)>=120
              success(cnt,1)=1;
          else
             success(cnt,1)=0;
          end
         ...
         meanoutputSNR(cnt,2) =20*log10(norm(x,2)/norm((x-x_hat2),2));
         if  meanoutputSNR(cnt,2)>=120
              success(cnt,2)=1;
         else
              success(cnt,2)=0;
         end
         [x_hat3,X_hat3,freq_hat3]=rd_recovery8(y3,t,S,N,M,Tx); % recover input signal rd_recovery4(y,t,S,N,M,Tx)
          meanoutputSNR(cnt,3) =20*log10(norm(x,2)/norm((x-x_hat3),2));
        if  meanoutputSNR(cnt,3)>=120
             success(cnt,3)=1;
        else
              success(cnt,3)=0;
        end
  ...
  su=sum(success); 
 successrate1(i,1)= 100*su(1)/(length(meanoutputSNR));

 successrate2(i,1)= 100*su(2)/(length(meanoutputSNR));
 
 successrate3(i,1)= 100*su(3)/(length(meanoutputSNR));

end

plot(Mt/N,successrate1,'-ro','Linewidth',2);
hold on;
plot(Mt/N,successrate2,'-b^','Linewidth',2);
hold on;
plot(Mt/N,successrate3,'-gs','Linewidth',2);
grid on
xlabel('compression rate');
ylabel('success reconstruction percentage(%)');
