...
%   http://dsp.rice.edu/sites/dsp.rice.edu/files/cs/asilomar08_final.pdf
    ...
    for ii=1:IterMax%最多迭代M次
        ...
        if norm(r_new)<20  %halting condition true 
            Pos_theta = F;
   ...
   orisampsnr=20*log10(norm_y/norm(r_new));
   if orisampsnr<=21
%        zhixing=1;
    [Pos_theta,theta_ls]=denoise(A,F,y);
%    
%    else
%        zhixing=0;
   end
%    switch(zhixing)
%        
%       [Pos_theta,theta_1s]=denoise(A,F,y);
